...
class Dataset:
    ...
    Attributes
    ----------
    alog : str          Path to the reference event log CSV.
    model : str         Path to the BPMN model used by the simulator.
    params : str        Path to the JSON simulation parameters.
    total_cases : int   Number of cases the simulator should generate.
    cut : str           Default cut-off timestamp (ISO-8601, UTC).
    horizon_days : int  Evaluation horizon in days after the cut-off.
    """
    alog: str
    model: str
    params: str
...
def _run_dataset(dataset: str, runs: int, *, cut_strategy: str) -> None:
    ...
    For each cut-off (determined by *cut_strategy*), repeats the simulation
    *runs* times for each flavour, aggregates metrics, and writes a
    ``final_results.json`` to ``outputs/<dataset>/<run_id>/``.
    """
