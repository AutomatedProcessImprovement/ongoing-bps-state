...
| 4 | **Compensating front/back XOR** (swap front-/back-loaded branches) | **timeline** | **State** (time-weighted); cycle_time/ngd **and red** 0.5 | 🔨 `front_back_load` *(redesign)* | to rebuild |
...
- **Process:** an XOR with **two branches of equal total duration** — one
  front-loaded, one back-loaded. 50/50 population.
- **Change (model):** **swap** the two loadings. Each case's total duration is
  unchanged.
