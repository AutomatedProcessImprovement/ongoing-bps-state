...
## Implemented oracle: `case_route` (real attribute-driven routing)

🟢 **case_route** — the simulator-driven upgrade of the `label_swap` oracle.
Instead of merging two independent basic sims and tagging them green/red
post-hoc, a single short-term simulation routes cases by a genuine `case_type`
∈ {red, blue} case attribute: red → A-branch, blue → B-branch at each ruled XOR
split (Prosimos `branch_rules`). Ongoing cases keep their historical `case_type`
because it is restored from the partial-state snapshot on resume (see
`src/attributes.py`), so attribute-driven routing stays consistent across the
cutoff.

- Dataset: `synthetic_case_route` — a chain of duration-symmetric XOR gateways
  separated by common "triage" activities, generated by
  `tools/generate_case_route.py`. Branch activities are never adjacent, so no
  2-gram couples two branch choices.
- Because each gateway's path marginal stays 50/50 and branches are
  duration-symmetric, `cycle_time`, `ngd_n2`, `red`, `rtd` and every
  type-agnostic state projection (`activity`, `case`, `cardinality`,
  `activity_role`) are blind. Only the `case_type`-aware projections detect.
- Measurement: the reference is the real attribute-routed continuation; `level`
  = percentage of `case_type` tags swapped on it (the pairing-unique attack,
  reusing `label_swap_case_types`). Result on a dense window (≈50 ongoing
  cases): `activity_type` c-index ≈ 0.94, `case_type` ≈ 0.88, everything else
  0.5 — matching the `label_swap` headline but on genuine simulator output.
- Run: `python -m evaluation.state_metrics.run_pipeline synthetic_case_route
  --perturbation case_route --cutoff-strategy n_ongoing --target-ongoing 45
  --horizon-hours 3`.

## Cross-references in this repo

- `evaluation/state_metrics/perturb.py` — existing pattern (`build_perturbed_params`, resource removal); `build_case_route_params` for the case_route oracle.
- `tools/generate_case_route.py` — generator for the `synthetic_case_route` BPMN + params.
