...
"""Evaluate-aggregate-compare pipeline for simulation quality metrics.

Provides the core ``evaluate()`` function that runs a simulation, splits the
output into event/ongoing/complete subsets, and computes distance metrics
against the reference log. ``aggregate()`` and ``compare()`` combine results
from multiple Monte-Carlo repetitions.
"""
...
def evaluate(
    ...
    *,
    ...
    """Run one simulation of the given *flavour*, build G subsets, and compute metrics.

    Parameters
    ----------
    flavour : str
        One of ``"process_state"``, ``"warmup"``, or ``"warmup2"``.
    io : SimulationIO
        Paths for the simulation's log/stats/output directory.
    sim_runner : callable
        Function that actually executes the simulation engine.

    Returns
    -------
    dict
        Nested dict ``{subfamily: {metric_name: value}}`` with three
        subfamilies: ``event_filter``, ``ongoing_filter``, ``complete_filter``.
    """
    if verbose:
...
def _metrics(A_event, A_ongoing, A_complete,
             ...
             cutoff) -> dict:
    """Compute all distance metrics between reference (A) and simulated (G) subsets.

    Returns a nested dict with keys ``event_filter``, ``ongoing_filter``,
    ``complete_filter``, each mapping metric names to float values (or None
    on failure).
    """
...
def aggregate(runs: list[dict]) -> dict:
    """Combine metric dicts from multiple runs into mean +/- CI per metric.

    Parameters
    ----------
    runs : list[dict]
        Each element is a nested dict ``{subfamily: {metric: value}}``.

    Returns
    -------
    dict
        ``{subfamily: {metric: {"mean": float, "ci": float}}}``.
    """
    res = {}
...
def compare(a: dict, b: dict, names=("A","B")) -> dict:
    """Compare two aggregated metric dicts, reporting which is better (lower) per metric.

    Returns
    -------
    dict
        ``{subfamily: {metric: {names[0]: x, names[1]: y, "better": name|"tie"|None}}}``.
    """
    out = {}
    for sf in a:
        out[sf] = {}
