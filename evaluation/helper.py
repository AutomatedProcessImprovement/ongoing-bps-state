...
"""Shared I/O, windowing, and subset-splitting utilities for the evaluation pipelines."""
...
def read_event_log(
    ...
    """Read a CSV event log, optionally renaming columns and parsing timestamps.
    ...
    rename : dict, optional
        Column rename mapping applied before any other processing.
    required : list[str], optional
        Columns that must be present after renaming; raises ValueError if missing.
    """
...
def _split_cases(df: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Partition cases into *ongoing* at the cutoff and *complete* within the window.

    Returns
    -------
    ongoing : DataFrame
        Events of cases that started before *start* and have not yet ended.
    complete : DataFrame
        All events of cases whose first event falls within [start, end].
    """
    ongoing, complete = [], []
